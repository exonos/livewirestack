...
<div x-data="{
    ...
    @if(!$file || $mode === 'profile')
        <label for="file-{{ $uuid }}" class="relative block leading-tight bg-white hover:bg-gray-100 cursor-pointer inline-flex items-center transition duration-500 ease-in-out group overflow-hidden
            {{  $mode === 'profile' ? 'border group '. $profileClass : 'border-2 w-full pl-3 pr-4 py-2 rounded-lg border-dashed' }}
        "
               wire:loading.class="pointer-events-none"
               :class="{ 'border-blue-300': isFocused === true }"
        >
            ...
            <div wire:loading.flex wire:target="{{ $attributes->wire('model')->value }}" wire:loading.class="w-full">
                @if ($mode === 'profile' && $file)
                    <div class="select-none text-sm text-blue-500 flex flex-1 items-center justify-center text-center p-4 flex-1">
                    ...
                    <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-1">
                                <div class="text-xs text-gray-500">{{ $bytesToHuman($f->getSize()) }}</div>
                ...
                @if($file)
                    <div class="mt-3 flex">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-1">
                                <div class="text-xs text-gray-500">{{ $bytesToHuman($file->getSize()) }}</div>
                                <div class="text-gray-400 text-xs">&bull;</div>
